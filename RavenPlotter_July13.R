...
# First, install warbleR from github if Marcelo hasn't updated the package
library(devtools)
...
#sort1 <- alltogether[order(alltogether$noteID),]    #sort by noteID
catalog(sort1[sort1$noteID == '0.0.0.1.1.1',],
        flim = c(0, 5),
        nrow = 10,
        ncol = 10,
        cex = 1,
...
system.time(catalog(sort1[sort1$species == 'Black-billed sicklebill',][1:200,], 
        flim = c(0, 5), 
        nrow = 10, 
        ncol = 10, 
        parallel = 2,
